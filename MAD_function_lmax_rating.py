        ...
        for user in test_movie_watched_by:
            ...
            for movie in test_user_movies_watched:
                if movie in user_movies_watched:
                    ...
                    lmax_list.append(abs(test_user_rating - user_rating))

            if no_of_common_movies != 0:
                lmax_coeff = max(lmax_list)
                weight = lmax_coeff
                users_ranking[user] = weight

        users_ranking_sorted = sorted(users_ranking.items(), key=lambda x: x[1])[:100]
